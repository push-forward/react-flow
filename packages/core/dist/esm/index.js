...
const selector$e = (s) => (s.userSelectionActive ? 'none' : 'all');
function Panel({ position, children, className, style, ...rest }) {
    const pointerEvents = useStore(selector$e);
...
const useViewportHelper = () => {
    const store = useStoreApi();
    const { d3Zoom, d3Selection } = useStore(selector$9, shallow);
...
const ZoomPane = ({ onMove, onMoveStart, onMoveEnd, zoomOnScroll = true, zoomOnPinch = true, panOnScroll = false, panOnScrollSpeed = 0.5, panOnScrollMode = PanOnScrollMode.Free, zoomOnDoubleClick = true, selectionKeyPressed, elementsSelectable, panOnDrag = true, defaultViewport, translateExtent, minZoom, maxZoom, zoomActivationKeyCode, preventScrolling = true, children, noWheelClassName, noPanClassName, }) => {
    ...
    const { d3Zoom, d3Selection, d3ZoomHandler } = useStore(selector$8, shallow);
    ...
    useEffect(() => {
        if (d3Selection && d3Zoom) {
            if (panOnScroll && !zoomActivationKeyPressed && !ctrlKeyPressed) {
...
const wrapHandlers = (handlers, containerRef) => Object.keys(handlers).reduce((hls, key) => ({ ...hls, [key]: wrapHandler(handlers[key], containerRef) }), {});
const selector$7 = (s) => ({
    userSelectionActive: s.userSelectionActive,
    elementsSelectable: s.elementsSelectable,
    paneDragging: s.paneDragging,
});
const UserSelection = memo(({ isSelectionMode, selectBoxMode = 'Contained', onSelectionStart, onSelectionEnd, onClick, onContextMenu, onWheel, onMouseEnter: onPaneMouseEnter, onMouseMove: onPaneMouseMove, onMouseLeave: onPaneMouseLeave, children, }) => {
    ...
    const { userSelectionActive, elementsSelectable, paneDragging } = useStore(selector$7, shallow);
    ...
    const onMouseMove = (event) => {
        ...
        const nodes = Array.from(nodeInternals.values());
        const selectedNodes = getNodesInside(nodeInternals, nextUserSelectRect, transform, selectBoxMode === 'Overlap', true, nodeOrigin);
    ...
    const eventHandlers = elementsSelectable && (isSelectionMode || userSelectionActive)
        ? {
    ...
    return (jsxs("div", { className: cc([
            'react-flow__pane',
            'react-flow__container',
            { dragging: paneDragging, selection: isSelectionMode },
        ]), ...eventHandlers, ref: container, style: containerStyle, children: [children, userSelectionActive && userSelectionRect && (jsx("div", { className: "react-flow__selection react-flow__container", style: {
...
const FlowRenderer = ({ children, onPaneClick, onPaneMouseEnter, onPaneMouseMove, onPaneMouseLeave, onPaneContextMenu, onPaneScroll, deleteKeyCode, onMove, onMoveStart, onMoveEnd, selectionKeyCode, selectBoxOnDrag, selectBoxMode, onSelectionStart, onSelectionEnd, multiSelectionKeyCode, zoomActivationKeyCode, elementsSelectable, zoomOnScroll, zoomOnPinch, panOnScroll, panOnScrollSpeed, panOnScrollMode, zoomOnDoubleClick, panOnDrag, defaultViewport, translateExtent, minZoom, maxZoom, preventScrolling, onSelectionContextMenu, noWheelClassName, noPanClassName, disableKeyboardA11y, }) => {
    const store = useStoreApi();
    ...
    return (jsx(ZoomPane, { onMove: onMove, onMoveStart: onMoveStart, onMoveEnd: onMoveEnd, onPaneContextMenu: onPaneContextMenu, elementsSelectable: elementsSelectable, zoomOnScroll: zoomOnScroll, zoomOnPinch: zoomOnPinch, panOnScroll: panOnScroll, panOnScrollSpeed: panOnScrollSpeed, panOnScrollMode: panOnScrollMode, zoomOnDoubleClick: zoomOnDoubleClick, panOnDrag: !selectionKeyPressed && panOnDrag, defaultViewport: defaultViewport, translateExtent: translateExtent, minZoom: minZoom, maxZoom: maxZoom, zoomActivationKeyCode: zoomActivationKeyCode, preventScrolling: preventScrolling, noWheelClassName: noWheelClassName, noPanClassName: noPanClassName, selectionKeyPressed: selectionKeyPressed, children: jsxs(UserSelection, { onSelectionStart: onSelectionStart, onSelectionEnd: onSelectionEnd, onClick: onClick, onMouseEnter: onPaneMouseEnter, onMouseMove: onPaneMouseMove, onMouseLeave: onPaneMouseLeave, onContextMenu: onContextMenu, onWheel: onWheel, isSelectionMode: !!isSelectionMode, selectBoxMode: selectBoxMode, children: [children, nodesSelectionActive && (jsx(NodesSelection$1, { onSelectionContextMenu: onSelectionContextMenu, noPanClassName: noPanClassName, disableKeyboardA11y: disableKeyboardA11y }))] }) }));
};
